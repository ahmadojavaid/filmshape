...
bool offStage = true;
BehaviorSubject<bool> txtShowData = BehaviorSubject();

class MaterialSearchResult extends StatefulWidget {
  final String value;
  final String text;
  final IconData icon;
  ...
  @override
  _MaterialSearchDataState createState() => _MaterialSearchDataState();
}

class _MaterialSearchDataState extends State<MaterialSearchResult> {
  @override
  Widget build(BuildContext context) {
    return StreamBuilder<bool>(
        stream: txtShowData.stream,
        builder: (context, snapshot) {
          snapshot.hasData ? offStage = snapshot.data : false;
          ...
          return Offstage(
            ...
            child: new Container(
              ...
              child: new Row(
                children: <Widget>[
                  new Container(width: 30.0, child: new Icon(widget.icon)),
                  new Expanded(
                      child: Container(
...
class _MaterialSearchState<T> extends State<MaterialSearch> {
  bool _loading = false;
  List<MaterialSearchResult> _results = [];
  ...
  Widget build(BuildContext context) {
    ...
    if (results.length == 0 || _criteria.toString().trim().length == 0) {
      txtShowData.add(true);
      print(false);
      print(results);
      print(_criteria);
    } else {
      txtShowData.add(false);
      print(true);
      print(results);
      print(_criteria);
    }
