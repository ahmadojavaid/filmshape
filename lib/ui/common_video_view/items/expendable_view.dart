...
class SampleExpendable extends StatefulWidget {

  ValueSetter<List<String>> voidcallback;
  int type;
  bool maxLimit;
...
class _MyAppState extends State<SampleExpendable> {
                  ...
                  return new ExpansionTile(
                    key: Key(
                        provider.rolesList[i].category.isExpend != null
                            ? provider
                            .rolesList[i].category.isExpend.toString()
                            : "false"),
                    initiallyExpanded: provider.rolesList[i].category
                        .isExpend !=
                        null ? provider.rolesList[i].category.isExpend : false,
    ...
    for (GetRolesResponse list in provider.rolesList) {
      ...
      for (RolesData childItem in rolesItem) {
        if (childItem.name == data) {
          ...
          if (widget.voidcallback != null && widget.type == 3) {
            widget.voidcallback(null);
          }
  ...
  Widget buildItem(RolesData item, int childPos, int parentPos) {
    ...
    widget.voidcallback(provider.list);
    ...
    return GestureDetector(
        onTap: () {
          ...
          if (item.isChecked) {
            ...
            if ((widget.maxLimit != null && widget.maxLimit) ||
              ...
              /*  showAlert(
                context: context,
                titleText: "ERROR",
                message: "Can't select upto 5 roles",
                actionCallbacks: {"OK": () {}},
              );*/
              showInSnackBar("5 role limit reached");
              item.isChecked = !item.isChecked;
            }
                  ...
                  onTap: () {
                    ...
                    if (item.isChecked) {
                      if ((widget.maxLimit != null && widget.maxLimit) ||
                        ...
                        provider.list.add(item.name);
                        provider.listId.add(item.id);
                        AddRoleModel addmodel = new AddRoleModel(title: item
                            .category.name,
                            iconurl: item.iconUrl,
                            list: item.name);
                        addRoleData(addmodel);
    ...
    for (String data in provider.list) {
      yield Container(
        ...
        child: Chip(
          ...
          label: Text(
            data, style: new TextStyle(color: AppColors.topNavColor),),
          deleteIcon: Icon(
            Icons.clear,
            size: 15.0,
  ...
  _buildContestList(List<RolesData> list,
    int parentPos,
  ) {
    return Container(
