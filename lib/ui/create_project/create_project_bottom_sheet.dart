...
import '../search_for_talent.dart';

class UserAssignedToRoleBottomSheet extends StatefulWidget {
  final ValueChanged<int> updateCallBack;
  final ProjectRoleCalls response;
  final String projectId;

  UserAssignedToRoleBottomSheet(this.updateCallBack, this.response,
      this.projectId);

  @override
  _UserAssignedToRoleBottomSheetState createState() =>
      _UserAssignedToRoleBottomSheetState();
}

class _UserAssignedToRoleBottomSheetState
    extends State<UserAssignedToRoleBottomSheet> {
  ...
  void initState() {
    ...
    title = widget.response.role.name;
    ...
    if (title == "Actor" || title == "Actor (Extra)") {
      _isActorRole = false;
        ...
        if (type == 1) {
          ...
          Navigator.push(
              ...
              new CupertinoPageRoute(builder: (BuildContext context) {
                return new SearchForTalent(previousTabHeading: projectName,);
              }));
        }
      },
      child: new Container(
      ...
      children: <Widget>[
        new ListView(
          children: <Widget>[
            ...
            getHideSalary(hideSalaryChangeNotifier),
...
//        RolesCreateProfile(type: "User", id: int.parse(id));
  ...
  removeRoleSelfAssigned() async {
    //update status;
    var roleId = widget.response.id;

    provider.setLoading();
    var response = await providers.unassignToSelf(
    ...
    if ((response is APIError)) {
      ...
      widget.updateCallBack(
          0); //update total count added for a role

    }
  }

  //assign role to self
  assignRoleToSelft() async {
    var roleId = widget.response.id.toString();
    provider.setLoading();
    var response = await providers.assignMySelf(
        context, roleId);
  ...
  void deleteRole() async {
    var roleId = widget.response.id;
    provider.setLoading();
    var response = await providers.removeTab(context, roleId.toString());
    provider.hideLoader();
